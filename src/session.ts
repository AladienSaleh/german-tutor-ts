...
async function* wsMessages(ws: WebSocket): AsyncGenerator<Buffer> {
  const queue: Buffer[] = [];
...
export async function handleSession(
  ...
  async function flush(gen: AsyncGenerator<object>) {
    for await (const msg of gen) send(msg);
  }
  ...
  try {
    ...
    for await (const raw of wsMessages(ws)) {
      ...
      if (durationSec(pcm) < MIN_AUDIO_SEC) continue;
      ...
      if (lastAssistant) dbSession.logTurn('assistant', lastAssistant.content);
    }
  } catch (err: unknown) {
